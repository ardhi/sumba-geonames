...
async function maintenance ({ type = 'modifications', headers, converterFn, date } = {}) {
  const { fs, print, importPkg, dayjs, generateId, spinner, startPlugin } = this.bajo.helper
  const { importFrom, download } = this.bajoExtra.helper
  ...
  let dest
  try {
    dest = await download(url.replace('{type}', type).replace('{date}', date), { spin, spinText: downloadText, fileName })
  } catch (err) {
  ...
  for (const i in data) {
    const d = data[i]
    const item = await recordFindOne(coll, { query: { geonamesId: d.geonamesId } }, { noHook: true, noCache: true })
